...
namespace VirtualTeacherGenAIDemo.Server.Services
{
    public class DashboardService
    {
        ...
        //Delete all dashboard from a sessionId
        public async Task DeleteDashboardAsync(string sessionId)
        {
            var dashboard = await _dashboardRepository.FindByChatIdAsync(sessionId);
            foreach (var item in dashboard)
            {
                await _dashboardRepository.DeleteAsync(item);
            }
        }
    }
}
